...
export class PyodidePythonEnvironment implements PythonEnvironment {
    ...
    async terminate(): Promise<void> {
        if (this.pyodide) {
            // Set interrupt buffer to trigger SIGINT (value 2) to terminate execution
            this.interrupt[0] = 2;
            
            // Give some time for the interrupt to take effect
            ...
            files.forEach((f) => {
                if (f.filename == undefined || f.b64_data == undefined) {
                    ...
                    result.error = {
                        type: "parsing",
                        message:
                            "file data is missing for: " + JSON.stringify(f),
                    };
                    return result;
                }
